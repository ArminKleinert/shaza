...
Variable toVector(Variable v) {
    Variable res = v.visit!((SzVector sv) => sv, //(SzList sl) => sl,
            // (SzMap sm) => // TODO
            other => null)();
...
Variable toMap(Variable v) {
    Variable res = v.visit!( // (SzVector sv) => , // TODO
            // (SzList sl) => , // TODO
            (SzMap sm) => sm, other => null)();
...
Variable add(Variable v0, Variable v1) {
    if (isIntegral(v0)) {
        if (isIntegral(v1))
            return variable(v0.get!(long) + v1.get!(long));
        else if (isNonIntNum(v1))
            return variable(v0.get!(long) + v1.get!(double));
    } else if (isNonIntNum(v0)) {
        if (isIntegral(v1))
            return variable(v0.get!(double) + v1.get!(long));
        else if (isNonIntNum(v1))
            return variable(v0.get!(double) + v1.get!(double));
    }
...
static void setup() {
    Namespace.globalNs.put("+", fnToVariable(&add));
    Namespace.globalNs.put("-", fnToVariable(&sub));
    ...
    Namespace.globalNs.put("/'", fnToVariable(&et_div));
    Namespace.globalNs.put("%'", fnToVariable(&et_mod));

    Namespace.globalNs.put("list", fnToVariable(&cr_list));
    Namespace.globalNs.put("vector", fnToVariable(&cr_vector));
    Namespace.globalNs.put("cr_map", fnToVariable(&cr_map));
}

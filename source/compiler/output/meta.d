    ...
    for (auto i = 0; i < attribs.size; i++) {
        ...
        if (attribs.nodes[i].text == ":generics") {
            ...
            if (attribs.nodes[i].type != TknType.closedList) {
                throw new CompilerError( "meta: Expected list for attribute tk-aliases. " ~attribs.nodes[i].tknstr);
            } else if (attribs.nodes[i].size % 2 != 0) {
                throw new CompilerError( "meta: tk-aliases requires even number of arguments. " ~ attribs.nodes[i].tknstr);
            }
            ...
            for (auto i2 = 0; i2 < aliasedStuff.size; i2 += 2) {
                ...
                if (!isAtom( orig.tkn) || !isAtom( replacement.tkn)) {
                    throw new CompilerError( "meta: All tokens in tk-aliases must be atomic. " ~ aliasedStuff.nodes[i2].tknstr);
                }
                if (ast.nodes.size > 2) {
